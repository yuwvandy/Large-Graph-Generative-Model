...
from analysis.spectre_utils import loader_to_nx, compute_metrics


class DiscreteDenoisingDiffusion(pl.LightningModule):
    def __init__(self, cfg, input_dims, output_dims, cond_dims, cond_emb, \
                 nodes_dist, node_types, edge_types, extra_features, data_loaders, sampling_metrics):
        super().__init__()
    ...
    def training_step(self, data, i):
        ...
        noisy_data = self.apply_noise(X, E, data.y, data.cond_emb, node_mask)
    ...
    def validation_step(self, data, i):
        ...
        dense_data = dense_data.mask(node_mask)
        noisy_data = self.apply_noise(dense_data.X, dense_data.E, data.y, data.cond_emb, node_mask)
        extra_data = self.compute_extra_data(noisy_data)
        pred = self.forward(noisy_data, extra_data, node_mask)
    ...
    #         to_save = min(samples_left_to_save, bs)
    #         chains_save = min(chains_left_to_save, bs)
    #         samples.extend(self.sample_batch(to_generate, num_nodes=None, cond_emb=self.cond_emb))
    #         id += to_generate
    #         samples_left_to_save -= to_save
    #         samples_left_to_generate -= to_generate
    #         chains_left_to_save -= chains_save
    ...
    def on_test_epoch_end(self) -> None:
        referenced_graphs = loader_to_nx(self.test_loader)

        generated_graphs = {}

        print(len(referenced_graphs.keys()))
        for key in tqdm(referenced_graphs.keys()):
            generated_graphs[key] = self.sample_batch(5, num_nodes=None, cond_emb=self.cond_emb[key])

        degrees, spectres, clusterings, orbits = compute_metrics(generated_graphs, referenced_graphs)
        self.print("Done testing.")
        print("Degrees: ", degrees)
        print("Spectres: ", spectres)
        print("Clusterings: ", clusterings)
        print("Orbits: ", orbits)

        return degrees, spectres, clusterings, orbits


    def kl_prior(self, X, E, node_mask, transition):
    ...
    def reconstruction_logp(self, t, X, E, node_mask, cond_emb):
        ...
        noisy_data = {'X_t': sampled_0.X, 'E_t': sampled_0.E, 'y_t': sampled_0.y, 'node_mask': node_mask,
                      't': torch.zeros(X0.shape[0], 1).type_as(y0), 'cond_emb': cond_emb}
        extra_data = self.compute_extra_data(noisy_data)
    ...
    def apply_noise(self, X, E, y, cond_emb, node_mask):
        ...
        # (bs, n, n, de_in) @ (bs, 1, de_in, de_out) -> (bs, n, n, de_out)
        probE = E @ Qtb.E.unsqueeze(1)  # (bs, n, n, de_out)
        ...
        noisy_data = {'t_int': t_int, 't': t_float, 'beta_t': beta_t, 'alpha_s_bar': alpha_s_bar,
                      'alpha_t_bar': alpha_t_bar, 'X_t': z_t.X, 'E_t': z_t.E, 'y_t': z_t.y, 'node_mask': node_mask, 'cond_emb': cond_emb}
        return noisy_data
    ...
    def compute_val_loss(self, pred, noisy_data, X, E, y, node_mask, test=False):
       ...
       """
        ...
        kl_prior = self.kl_prior(X, E, node_mask, transition = self.cfg.model.transition)
        ...
        prob0 = self.reconstruction_logp(t, X, E, node_mask, noisy_data['cond_emb'])
    ...
    @torch.no_grad()
    def sample_batch(self, batch_size: int, num_nodes=None, cond_emb = None):
        """
        :param batch_id: int
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask, cond_emb):
        ...
        noisy_data = {'X_t': X_t, 'E_t': E_t, 'y_t': y_t, 't': t, 'node_mask': node_mask, 'cond_emb': cond_emb.unsqueeze(0).repeat(X_t.shape[0], 1)}
        extra_data = self.compute_extra_data(noisy_data)
        pred = self.forward(noisy_data, extra_data, node_mask)

...
def compute_metrics(generated_graphs, referenced_graphs):
    ...
    degree = degree_stats(referenced_graphs, nx_graphs, is_parallel=True, compute_emd=False)
    spectre = spectral_stats(referenced_graphs, nx_graphs, is_parallel=True, n_eigvals=-1, compute_emd=False)
    clustering = clustering_stats(referenced_graphs, nx_graphs, bins=100, is_parallel=True, compute_emd=False)
    orbit = orbit_stats_all(referenced_graphs, nx_graphs, compute_emd=False)
    # for key in generated_graphs:
    #     degrees.append(degree_stats(referenced_graphs[key], networkx_graphs[key], is_parallel=True, compute_emd=False))
    #     spectres.append(spectral_stats(referenced_graphs[key], networkx_graphs[key], is_parallel=True, n_eigvals=-1, compute_emd=False))
    #     clusterings.append(clustering_stats(referenced_graphs[key], networkx_graphs[key], bins=100, is_parallel=True, compute_emd=False))
    #     orbits.append(orbit_stats_all(referenced_graphs[key], networkx_graphs[key], compute_emd=False))
    
    return degree, spectre, clustering, orbit
